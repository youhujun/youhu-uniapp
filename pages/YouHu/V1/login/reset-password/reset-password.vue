		...
		<view class="yh-page yh-height">
			...
			<yh-login-input class="login-input-phone-box">
				...
				<template v-slot:content>
					<view class="content-box-phone">
						<input class="input-phone" v-model="resetPasswordForm.phone" type="text" placeholder="请输入手机号" placeholder-class="placeholder-text" />
					</view>
				</template>
			</yh-login-input>
			<!-- 填写手机号结束 -->
			...
			<yh-login-input class="login-input-code-box">
				...
				<template v-slot:content>
					<view class="content-box-code">
						<input class=" input-code" v-model="resetPasswordForm.code" type="text" placeholder="请输入验证码" placeholder-class="placeholder-text" />
					</view>
				</template>
			...
			<yh-commit-button class="commit-button">
				<template v-slot:word>
					<view @tap="handleToRestPassword()"><text class="commit-button-word">找回密码</text></view>
				</template>
			</yh-commit-button>
			<!-- 提交按钮结束 -->
			<!-- 菜单tool 结束 -->
...
<script>
  ...
  import { validPhone, validCode, validPassword } from '@/utils/validate';
   import { sendPasswordCode,restPasswordByPhone} from '@/api/login/login'
  export default 
  {
    //计算属性
    computed:
    ...
    methods:{
		...
		handleToGetCode: debounce(function() {
			//console.log(this.resetPasswordForm.phone) //打印手机号
			//检测手机号
			const validResult = validPhone(this.resetPasswordForm.phone);
			if (validResult) {
				this.codeStatus = true;
		
				const result = sendPasswordCode(this.resetPasswordForm);
			} else {
				notice('请填写正确手机号');
			}
		}, 1000),
		...
		//重置密码
		handleToRestPassword()
		{
			let validResult = true;
			//console.log(this.resetPasswordForm)
			const validPhoneResult = validPhone(this.resetPasswordForm.phone);
			const validCodeResult = validCode(this.resetPasswordForm.code);
			const validPasswordResult = validPassword(this.resetPasswordForm.password);
			...
			if (validResult)
			{
				...
				restPasswordByPhone(this.resetPasswordForm).then(res=>
				{
					if(res && res.code === 0)
					{
						uni.reLaunch({
